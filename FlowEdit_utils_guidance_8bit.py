...
class MetricGuidance:
    def __init__(self, pipe, device,grad_metric_calculator):
        self.pipe = pipe
        self.device = device
        self.metric_calculator = grad_metric_calculator  # 指标计算器
...
def FlowEditFLUX(pipe,
    ...
    grad_metric_calculator = DifferentiableMetrics()
        ...
        if T_steps - i > n_min:
            ...
            for k in range(n_avg):
                ...
                if i % guide_freq == 0:  # 每3步应用一次指标引导
                    metric_guide = MetricGuidance(pipe, device='cuda',grad_metric_calculator=grad_metric_calculator)
