...
#****************************************************************
    ...
    for i, t in tqdm(enumerate(timesteps)):
        ...
        if T_steps - i > n_min:
            ...
            # 校正zt_tar（使用欧拉方法）
            zt_tar = zt_tar.to(torch.float32) + (t_im1 - t_i) * (Vt_tar - Vt_src)
            zt_tar = zt_tar.to(x_src_packed.dtype)
            ...
            # 更新ODE状态（使用欧拉方法）
            zt_edit = zt_edit.to(torch.float32) + (t_im1 - t_i) * (Vt_tar - Vt_src)
            zt_edit = zt_edit.to(x_src_packed.dtype)
